    ...
    if (RAZORPAY_KEY) {
      ...
      if (process.env.NODE_ENV === 'development') {
        toast({
          ...
          description: "Using test Razorpay key. Set VITE_RAZORPAY_KEY_ID in .env for production.",
          variant: "default"
        });
      }
    }
  ...
  const handleRazorpayPayment = async () => {
    if (!razorpayLoaded) {
      toast({
        ...
        description: "Please wait for the payment system to initialize or refresh the page.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      await initializeRazorpayPayment({
        ...
        prefill: {
          name,
          email: '', // Optional: Add email field if needed
          contact: phone,
        },
        notes: {
          consultationType,
        ...
        theme: {
          color: "#0284C7", // sociodent-600 color
        },
        handler: (response) => {
      ...
      // If it's a payment failure (not initialization failure), we might want to let the user try again
      if (error.message?.includes('payment')) {
        setIsProcessingPayment(false);
      } else {
        // For more serious errors, we might want to go back to step 1
        setStep(1);
        setIsProcessingPayment(false);
      }
    }
              ...
              <div className="glass-card border border-white/50 rounded-2xl overflow-hidden shadow-md bg-white">
                {isLoadingRazorpay ? (
                  <div className="p-6 text-center">
                    <Loading />
                    <p className="text-gray-600 mt-4">Initializing payment system...</p>
                  </div>
                ) : (
                  <>
                    {step === 1 && (
